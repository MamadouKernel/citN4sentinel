...
Write-Host "     puis Verifier-N4Sentinel.ps1 -Port $Port"
Write-Host ""
Write-Host "     N'EN RESTEZ PAS AU STATUT 'Running'. C'est precisement ce que" -ForegroundColor Yellow
Write-Host "     N4 Sentinel reproche a la supervision naive : un service qui se"
Write-Host "     declare demarre ne prouve pas qu'il rend le service. Le processus"
Write-Host "     peut tourner alors que la base est injoignable ou qu'une migration"
Write-Host "     a echoue. Le script de verification interroge /health, qui repond"
Write-Host "     apres l'amorcage reel."
Write-Host ""
Write-Host "     Ensuite : http://$env:COMPUTERNAME`:$Port"
